...
export const MemberList: FC<MemberListProps> = (props: MemberListProps): ReactElement => {
  // State variable to hold input values and flags
  const [isChannelInviteModalVisible, setIsChannelInviteModalVisible] = useState(false);
  const [channelInviteInput, setChannelInviteInput] = useState('');
  const [isChannelDeleteModalVisible, setIsChannelDeleteModalVisible] = useState(false);
  const [isChannelLeaveModalVisible, setIsChannelLeaveModalVisible] = useState(false);

  // Shows modal
  const showChannelInviteModal = (): void => {
    setIsChannelInviteModalVisible(true);
  }

  // Clears input and hide modal on cancel
  const handleChannelInviteModalCancel = (): void => {
    setChannelInviteInput("");
    setIsChannelInviteModalVisible(false);
  }

  // Invites an user to this channel
  const doInviteChannel = async (): Promise<boolean> => {
    const username: string = channelInviteInput;
    ...
    const invitedUserQuery: Parse.Query = new Parse.Query(Parse.User);
    ...
    try {
      ...
      const invitedUser: (Parse.Object | undefined) = await invitedUserQuery.first();
      if (invitedUser === undefined) {
        return false;
      }
      
      const Channel: Parse.Object = props.currentChannel;
      // Check if user is already in members
      if (Channel.get("members").find((member: Parse.User) => member.id === invitedUser.id) !== undefined) {
  ...
  const doDeleteChannel = async (): Promise<boolean> => {
    ...
    try {
      ...
      const messageQuery: Parse.Query = new Parse.Query('Message');
      messageQuery.equalTo("channel", Channel.toPointer());
      try {
        const messages: Parse.Object[] = await messageQuery.find();
      ...
      <div>
        {props.currentUser.id === props.currentChannel.get("owner").id ? (
          <>
            <div className="info-header" onClick={showChannelInviteModal}>
              ...
              <svg className="info-header__icon" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#000000"><path d="M0 0h24v24H0z" fill="none"/><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>
            </div>
            <Modal
            ...
            >
              <>
                ...
                <input
                  ...
                  onChange={(event: any) => setChannelInviteInput(event.target.value)}
                ></input>
              </>
            </Modal>
          </>
        ...
        <ul className="info-members">
          {props.currentChannel.get("members") && (
            <p>
              {props.currentChannel.get("members").map((result: Parse.User) => `${result.getUsername()}`).join(', ')}
            </p>
          )}
        </ul>
      </div>
      {props.currentUser.id === props.currentChannel.get("owner").id ? (
        <>
          <div className="info-buttons">
            <button className="info-button info-button--red" onClick={showChannelDeleteModal}>
              <span className="info-button__label">Delete channel</span>
              <svg className="info-button__icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg>
            </button>
          </div>
          <Modal
            ...
            okText={"Yes"}
          >
            <p>{"Are you sure that you want to delete this channel? All messages in it will be deleted!"}</p>
          </Modal>
        </>
      ) : (
        <>
          ...
          <Modal
            ...
            okText={"Yes"}
          >
            <p>{"Are you sure that you want to leave this channel? All your messages in it will be deleted!"}</p>
          </Modal>
        </>
      )}
    </>
